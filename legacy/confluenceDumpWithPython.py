...
if args.mode == "single":
    ...
    print(
        f"Exporting a Page Properties page with all its children (Sphinx set to {args.sphinx})"
    )
...
# For PAT tokens, leave user_name empty. For API tokens, use your email.
user_name = os.environ.get("atlassianUserEmail", "")
...
if args.mode == "single":
    ...
    my_body_export_view = myModules.get_body_export_view(
        atlassian_site, page_id, user_name, api_token
    ).json()
    ...
    my_outdir_base = os.path.join(
        my_outdir_base, f"{page_id}-{my_body_export_view_title}"
    ...
    all_spaces_full = myModules.get_spaces_all(
        atlassian_site, user_name, api_token
    ...
    if space_key == "" or space_key is None:  # if the supplied space key can't be found
        ...
        space_title = myModules.get_space_title(
            atlassian_site, space_id, user_name, api_token
        )
        #
        ...
        for p in all_pages_short:
            ...
            my_body_export_view = myModules.get_body_export_view(
                atlassian_site, p["page_id"], user_name, api_token
            ).json()
    ...
    my_outdir_content = os.path.join(
        my_outdir_base, str(page_id) + "-" + str(my_report_export_view_title)
    )
    # print("my_outdir_base: " + my_outdir_base)
    # print("my_outdir_content: " + my_outdir_content)
    if args.sphinx is False:
    ...
    for p in my_page_properties_children:
        ...
        my_child_export_view = myModules.get_body_export_view(
            atlassian_site, p, user_name, api_token
        ).json()
