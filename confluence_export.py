...
Usage:
    ...
    python confluenceDumpWithSSO.py --mode single --site confluence.example.com --page 123456 --html
    
    # Export an entire space
    python confluenceDumpWithSSO.py --mode space --site confluence.example.com --space MYSPACE
...
try:
    ...
    print("Install with: pip install browser-cookie3 requests beautifulsoup4 pypandoc Pillow")
    sys.exit(1)
...
class ConfluenceExporter:
    ...
    def get_page_tree(self, root_page_id: str, max_workers: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        def fetch_children(page_id: str, level: int = 0) -> List[Dict[str, Any]]:
            ...
            # Get children
            children_url = f"https://{self.site}/rest/api/content/{page_id}/child/page?limit=250"
                ...
                if child_pages:
                    ...
                    with ThreadPoolExecutor(max_workers=max_workers) as executor:
                        ...
                        for child in child_pages:
                            child_id = child['id']
                            future = executor.submit(fetch_children, child_id, level + 1)
    ...
    def build_folder_structure(self, pages: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        def get_path(page_id: str) -> str:
            ...
            page_paths[page_id] = os.path.join(parent_path, page_folder) if parent_path else page_folder
            return page_paths[page_id]
    ...
    def export_pages(self, pages: List[Dict[str, Any]], page_paths: Dict[str, str],
                    base_dir: str, sphinx: bool, tags: bool, html_output: bool, 
                    rst_output: bool, max_workers: int = 5):
        """
        ...
        def export_single(page: Dict[str, Any], page_num: int):
            ...
            if page['level'] == 0:
                ...
                page_outdir = os.path.join(base_dir, parent_path) if parent_path else base_dir
                os.makedirs(page_outdir, exist_ok=True)
            
            print(f"[{page_num}/{len(pages)}] {page['title']}")
            
            # Get metadata
            page_labels = myModules.get_page_labels(self.site, page_id, self.user_name, self.api_token)
            page_parent = myModules.get_page_parent(self.site, page_id, self.user_name, self.api_token)
            
            # Export
            myModules.dump_html(
                self.site, page_html, page_title, page_id,
                base_dir, page_outdir, page_labels, page_parent,
                self.user_name, self.api_token, sphinx, tags,
                arg_html_output=html_output, arg_rst_output=rst_output
            )
...
def get_browser_session(domain: str) -> requests.Session:
    ...
    raise Exception("Could not load cookies from any browser. Make sure you're logged into Confluence.")
...
Examples:
  ...
  %(prog)s --mode single --site confluence.example.com --page 123456 --html
  
  # Export entire space
  %(prog)s --mode space --site confluence.example.com --space MYSPACE
        '''
    )
    
    parser.add_argument('--mode', '-m', choices=['single', 'space'], required=True,
                       help='Export mode: single page with children, or entire space')
    ...
    parser.add_argument('--space', '-s', type=str,
                       help='Space key (required for space mode)')
    parser.add_argument('--page', '-p', type=str,
                       help='Page ID (required for single mode)')
    parser.add_argument('--outdir', '-o', type=str, default='output',
                       help='Output directory (default: output)')
    parser.add_argument('--sphinx', '-x', action='store_true',
                       help='Sphinx compatible folder structure')
    parser.add_argument('--tags', action='store_true',
                       help='Add labels as tags in RST files')
    parser.add_argument('--html', action='store_true',
                       help='Include HTML files in export')
    ...
    # Validate arguments
    if args.mode == 'single' and not args.page:
        parser.error("--page is required for single mode")
    if args.mode == 'space' and not args.space:
        parser.error("--space is required for space mode")
    ...
    if not test_authentication(session, domain):
        print("ERROR: Authentication failed. Please login to Confluence in your browser and try again.")
        sys.exit(1)
    ...
    if args.mode == 'single':
        ...
        root_title = pages[0]['title'].replace("/", "-").replace(",", "").replace("&", "And").replace(":", "-")
        ...
        exporter.export_pages(
            pages, page_paths, base_dir,
            args.sphinx, args.tags, args.html, args.rst,
            max_workers=args.workers
        )
        
    elif args.mode == 'space':
        print("ERROR: Space mode not yet fully implemented in refactored version")
        print("Use: python confluenceDumpWithSSO.py --mode space ...")
        sys.exit(1)
